...
$installerUrl = "https://github.com/msys2/msys2-installer/releases/download/2025-12-13/msys2-x86_64-20251213.exe"
$installPath = "C:\msys64"
#
$tempInstaller = "$env:TEMP\msys2-installer.exe"
$msysDir = "C:\msys64"
$executablePath = Join-Path -Path $msysDir -ChildPath "msys2.exe"
$toolchainPackage = "mingw-w64-ucrt-x86_64-gcc" # This package includes gcc, g++, etc.
...
# --- Step 3: Update the core system and install UCRT64 toolchain ---
# Note: MSYS2 requires updates to be run within its bash environment
Write-Host "Updating MSYS2 core packages and installing UCRT64 toolchain..."

# Define the path to the MSYS2 bash executable
$msysBash = "$installPath\usr\bin\bash.exe"
#$msysBash = "C:\msys64\msys2_shell.cmd -ucrt64 -defterm -nostart -here -c" 
...
Write-Host "Installing the UCRT64 GCC toolchain..."
$ucrt64_pkg = "mingw-w64-ucrt-x86_64-gcc"
& $msysBash -lc  "pacman -S --noconfirm --needed base-devel mingw-w64-ucrt-x86_64-toolchain"
& $msysBash  "pacman --noconfirm -S $ucrt64_pkg --needed"

Write-Host "Running final pacman -Syu update..."
& $msysBash lc  'pacman --noconfirm -Syu'
Write-Host "MSYS2 UCRT64 environment is ready. You can now use the 'MSYS2 UCRT64' shortcut in the Start menu."
