...
g++ Tone1k_14-1_float.cpp -o Tone1k_14-1_float.exe
*/
...
int main() {
    ...
    std::vector<std::complex<float>> iqSignals(numSamples);
    ...
    for (int n = 0; n < numSamples; ++n) {
        ...
        I = cos(phaseTone + phaseCarrier);
        Q = sin(phaseTone + phaseCarrier);
		iqSignals[n]= std::complex<float>(I, Q);
	...
	std::ofstream outFile("Tone1K_14-1_float.bin", std::ios::out | std::ios::binary);
    if (!outFile.is_open()) {
